...
const char * const ECOMMERCECOIN_CONF_FILENAME = "ecommercecoin.conf";
const char * const ECOMMERCECOIN_PID_FILENAME = "ecommercecoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "ecommercecoind -foo=bar
...
#else
    const char* pszModule = "ecommercecoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Ecommercecoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Ecommercecoin
    // Mac: ~/Library/Application Support/Ecommercecoin
    // Unix: ~/.ecommercecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Ecommercecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Ecommercecoin";
#else
    // Unix
    return pathRet / ".ecommercecoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", ECOMMERCECOIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", ECOMMERCECOIN_PID_FILENAME)));
}
