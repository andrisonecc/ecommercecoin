...
###   ecommercecoincore.org and ecommercecoin.org and compares them.
...
WORKINGDIR="/tmp/ecommercecoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://ecommercecoincore.org"
HOST2="https://ecommercecoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="ecommercecoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'ecommercecoin-',
...
if [ $? -ne 0 ]; then
   echo "ecommercecoin.org failed to provide signature file, but ecommercecoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "ecommercecoin.org and ecommercecoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Ecommercecoin Core binary release signing key installed?"
   fi
